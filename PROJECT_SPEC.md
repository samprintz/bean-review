...
- `import_cmd` is configured in the `[general]` section of the config file.
  There is no default; the key is inert until a command is configured.
...
- `archive_cmd` is configured in the `[general]` section of the config file.
  There is no default; the key is inert until a command is configured.
